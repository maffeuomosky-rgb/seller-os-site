import { consumeDownload, getOrderByCustomerToken } from '../lib/db.js';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!id || !token || token.length < 20) return json(res, 400, { error: 'INVALID_ORDER_AUTH' });
    ...
    console.error('order-download', error);
